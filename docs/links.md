...
word — the same vocabulary is drawn as a graph at the top of the
[vocabulary page](vocabulary.md), generated from the resolved schema rather
than hand-drawn, so it can't quietly go stale the way a hand-maintained
diagram (or table) does the first time a preset or project overlay changes a
verb. It is plain SVG: no script, no Mermaid, no renderer to install, and it
follows the site's theme in dark mode and on paper.

Any project can draw its own — its own overlay and presets included — with
`refdes schema --graph > graph.svg`, which writes the identical SVG to
stdout.

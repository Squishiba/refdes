...
A check compares a computed value against a bound's declared limit, at build
time. This is the mechanism that turns a document into something that can be wrong.

## Declaring a limit

Any item with a `limit` field can be checked against. In the standard schema that
is the `bound` type:
...
```
ERROR items/bounds/thermal.yaml:8 [BND-THM-001] — limit: could not read limit
...
sentence that reads naturally as "X, with Y, across Z" is not one bound with
three clauses; it is three bounds that happen to share a paragraph in the
datasheet.
...
This is strictly more useful than the merged version: each bound now checks
...
the result against an ordinary scalar bound:
...
checks:
  - value: I_total
    against: BND-PWR-020   # limit: "<= 25 A"
```

The relation (`+`) lives in the calc expression; the bound stays a single
...
| `check against 'BND-X', which does not exist` | wrong ID, or a failed import |
| `check against BND-X, which declares no limit` | target has no `limit` field |
...
A bound owned by a shared platform project can be checked by every board that
